...
use crate::{result::wrap_failable, value::GoLuaValue};
use std::sync::RwLock;
...
#[unsafe(no_mangle)]
pub extern "C" fn luago_create_multivalue_with_capacity(cap: usize) -> *mut GoMultiValue {
    wrap_failable(|| {
...
#[unsafe(no_mangle)]
pub extern "C" fn luago_multivalue_push(vec_ptr: *mut GoMultiValue, value: GoLuaValue) {
    wrap_failable(|| {
...
#[unsafe(no_mangle)] 
pub extern "C" fn luago_multivalue_len(vec_ptr: *mut GoMultiValue) -> usize {
    wrap_failable(|| {
        // Safety: Assume vec_ptr is a valid, non-null pointer to a LuaValueVec
        if vec_ptr.is_null() {
            panic!("LuaValueVec pointer is null");
...
#[unsafe(no_mangle)]
pub extern "C" fn luago_multivalue_pop(vec_ptr: *mut GoMultiValue) -> GoLuaValue {
    wrap_failable(|| {
...
#[unsafe(no_mangle)]
pub extern "C" fn luago_free_multivalue(vec_ptr: *mut GoMultiValue) {
    wrap_failable(|| {
        // Safety: Assume vec_ptr is a valid, non-null pointer to a GoMultiValue
        if vec_ptr.is_null() {
            return;
